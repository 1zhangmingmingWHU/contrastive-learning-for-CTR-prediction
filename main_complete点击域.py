...
from model_complete import ESMM
from basic.features import DenseFeature, SparseFeature
import torch
import os
from trainers.trainer_complete_click import MTLTrainer 
...
parser.add_argument("--tau", type=float, help="", default=5, required=False)
parser.add_argument("--earlystop_patience", type=int, help="", default=10)
...
zero_ctr_label_indices = df_test[df_test['click'] == 0].index#找到所有ctr为0的索引
...
model.load_state_dict(torch.load('/home/zmm/HUAWEIBEI/code/saved_models/08_03_09_27/model.pth'))
...
mtl_trainer = MTLTrainer(model=model, user_features=user_features, item_features=item_features, task_types=task_types, #用加了对比学习的trainer
              optimizer_params={"lr": args.lr, "weight_decay": args.weight_decay}, 
              n_epoch=epoch, earlystop_patience=args.earlystop_patience, device=device, 
              model_path=save_dir, alpha = args.alpha, tau = args.tau, sub_batch_size=args.sub_batch_size)
# mtl_trainer.fit(train_dataloader, val_dataloader)

# mtl_trainer.fit(train_dataloader, test_dataloader) #用测试集作为验证集
auc = mtl_trainer.evaluate(mtl_trainer.model, test_dataloader, zero_ctr_label_indices)
print("auc: ", auc)
