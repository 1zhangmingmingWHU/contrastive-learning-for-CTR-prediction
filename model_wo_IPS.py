...
class ESMM(nn.Module):
    ...
    def __init__(self, user_features, item_features, dense_num, cvr_params, ctr_params):
        super().__init__()
        self.cross_num = 4
        ...
        # self.tower_dims = len(self.user_features) * user_features[0].embed_dim + len(self.item_features) * item_features[0].embed_dim + self.dense_tower_dims
        ...
        # input_dim = 8
        # output_dim = 8
        # self.linear_C_0 = nn.Linear(input_dim, output_dim)
        # self.linears_C_l = nn.ModuleList([nn.Linear(output_dim, output_dim) for _ in range(self.num_layers)])
    ...
    def forward(self, x):
        ...
        # C_1 = self.linear_C_0(torch.mm(embed_dense_features, embed_dense_features.T))
        # # Concatenate O_C_1
        # O_C_1 = torch.cat((embed_dense_features, C_1), dim=1)
        ...
        #     # Compute C_l+1
        #     C_l_plus_1 = self.linears_C_l[l](torch.mm(embed_dense_features, C_l.T))
        #     # Concatenate O_G_l
        #     O_C_l = torch.cat((O_C_l, C_l_plus_1), dim=1)
        #     C_l = C_l_plus_1
        ...
        for i in range(self.cross_num):
            ...
            x_1 = x_2
        # O_C_l = torch.cat([cross_output, embed_dense_features], dim=-1)


        
        # embed_dense_features = embed_dense_features.reshape(bsz, -1)
        # print(embed_dense_features)
        # dense_fea = self.dense_tower(embed_dense_features)
