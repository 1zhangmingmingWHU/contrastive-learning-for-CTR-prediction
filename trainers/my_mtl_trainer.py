...
class MTLTrainer(object):
    ...
    # def InfoNCE(self, view1, view2, temperature: float, b_cos: bool = True): #直接用batch的大小，会导致对比学习很差
    #     if b_cos:
    #         view2 = F.normalize(view2, dim=1)
    #         view1 = F.normalize(view1, dim=1)
    #     # 沿着 batch_size 维度拼接两个张量
    #     concatenated_tensor = torch.cat((view1, view2), dim=0)  #[batch_size*2, num_features*embed_dim]
    #     # Compute similarity scores
    #     similarity_matrix = (concatenated_tensor @ concatenated_tensor.T) / temperature
    #     # 将对角线部分设置为负无穷
    #     similarity_matrix.fill_diagonal_(float('-inf'))
    #     #计算batch_size
    #     batch_size = similarity_matrix.size(0) // 2 #矩阵的大小是2*batch_size
    #     # 取出下半部分的矩阵
    #     lower_half = similarity_matrix[batch_size:, :]
    #     # 取出右半部分的矩阵
    #     right_half = similarity_matrix[:, batch_size:]
    #     # 计算下半部分的log_softmax
    #     lower_log_softmax_scores = F.log_softmax(lower_half, dim=1)
    #     # 计算右半部分的log_softmax
    #     right_log_softmax_scores = F.log_softmax(right_half, dim=0)
    #     # 获取下半部分的log_softmax的左半部分
    #     left_half_lower_log_softmax_scores = lower_log_softmax_scores[:, :batch_size]
    #     # 获取右半部分的log_softmax的上半部分
    #     uper_half_right_log_softmax_scores = right_log_softmax_scores[:batch_size, :]
    #     # 取两个对角线
    #     pos_log_softmax_scores = torch.diag(left_half_lower_log_softmax_scores) + torch.diag(uper_half_right_log_softmax_scores)
    #     # 计算loss
